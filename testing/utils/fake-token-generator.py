# fake-token-generator.py
#
# * Generates technically valid but practically fake Bearer tokens.
# * The tokens can be used against Java & NodeJS services for testing purposes.
# * The tokens have valid payloads for rapid testing but may include unauthorized signatures for production.
#
# Usage: python fake-token-generator.py any_user_id
#

from time import time as now
import sys
import jwt

PRIV_KEY = b"-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----"
PUBL_KEY = b"-----BEGIN PUBLIC KEY-----\nMIGeMA0GCSqGSIb3DQEBAQUAA4GMADCBiAKBgGYsQ8X8QfxM5aPUGnqLLgQjqg3T\nbFoHiYtAf16N6j1MfxN/u9TE19GprcSBTOGLd25drS5vbXuNfDGTyvjtRCGkQvRO\nIeVTNLCC48zQwwnGBaK6hy0ssccyY9/dhdsmPteEg1rWG8+fIlP4lqNFwmpUluD7\nyCKc3bYHvmOoHFlXAgMBAAE=\n-----END PUBLIC KEY-----"

user_id = sys.argv[1]
print(f"🥳 Generating Bearer token with user ID of '{user_id}' (expires in 1 year):")

one_year_in_sec = 31556926
iat = now()
exp = iat + one_year_in_sec

token = jwt.encode(
    {
        "https://api.cryptoofun/roles": ["user"],
        "iss": "https://cryptoofun.eu.auth0.com/",
        "sub": user_id,
        "aud": ["https://api.cryptoofun", "https://cryptoofun.eu.auth0.com/userinfo"],
        "iat": iat,
        "exp": exp,
        "azp": "MhwgBUTyT4H0xd1hO7xq3zTyoque8UOy",
        "scope": "openid profile email",
        "permissions": ["read:services", "write:services"],
    },
    PRIV_KEY,
    algorithm="RS256",
)
print(token)
