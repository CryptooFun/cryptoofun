...
describe('trading-sell', () => {
  ...
  it('sell BTC with market order', () => {
    cy.visit(`${Cypress.config('baseUrl')}/trade/BTC_USDT`);
    cy.get('#sell-amount').clear().type(sellAmount);

    cy.intercept('POST', '**/trade/market/sell').as('postOrder');
  ...
  it('visit “View Orders Page”', () => {
    ...
    cy.get('table')
      ...
      .eq(3)
      .should('satisfy', $el => {
        return $el.text().includes(sellAmount.toString());
      });
    ...
    cy.get('table')
      ...
      .then(val => {
        ...
        cost = actualizationPrice * sellAmount * -1;
